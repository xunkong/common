...
public class HoyolabClient
{
    ...
    /// <exception cref="ArgumentException"><c>cookie</c> 中不包含 account_mid_v2 或 ltoken_v2</exception>
    public async Task<HoyolabUserInfo> GetHoyolabUserInfoAsync(string cookie, CancellationToken? cancellationToken = null)
    {
        ...
        if (!(cookie.Contains("account_mid_v2") && cookie.Contains("ltoken_v2")))
        {
            throw new ArgumentException(@"Cookie does not contain ""account_mid_v2"" or ""ltoken_v2"".");
        }
